...
class BasketService {
    ...
    async getBasket(){
        try{
            ...
            console.log("getCurrentBasket called, basket found:", basket);
            if(basket){
    ...
    async addItemToBasket(item: Product, quantity = 1, dispatch: Dispatch){
        try{    
            ...
            if(!basket){
                console.log("No basket found, creating a new basket.");
                basket = await this.createBasket();
            }
            else {
                console.log("Existing basket found:", basket);
            }
    ...
    private getCurrentBasket() {
      ...
      private async createBasket(): Promise<Basket> {
        try {
            ...
            // Create a new basket with a user-specific ID
            ...
            console.log("basketid"+newBasket.id);
            return newBasket;
        } catch (error) {
            ...
            if (error instanceof Error) {
                ...
                throw new Error("An unknown error occurred while creating the basket.");
            }
        }
    }
